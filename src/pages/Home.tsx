...
function getRandomProjects(projectsList: Project[], count: number, exclude: { id: string | number }[] = []) {
  const available = projectsList.filter((p) => !exclude.some((e) => e.id === p.id))
...
const Home = () => {
  ...
  const [displayed, setDisplayed] = useState(() => getRandomProjects(projectsList, 3))
      ...
      <div className="hero-section">
        <div className="hero-left">
          <div className="profile-img-wrapper">
            <img src={photoImg || "/placeholder.svg"} alt="Profile" className="profile-img" />
        ...
        <div className="hero-right">
          ...
          <p>
            A passionate developer dedicated to crafting professional, functional, and modern web applications that make
            a difference.
          </p>
      ...
      <div className="featured-projects">
        ...
        <div className="home-projects-list">
          {displayed.map((project, idx) => (
            <div
              key={`${flipCount}-${idx}`}
              className={`project-flip-container${flipping ? " flipping" : ""}${idx === 1 ? " lean-right" : ""}`}
            >
              <div className="project-flip-inner">
                ...
                <div className="project-flip-back">
                  {nextDisplayed[idx] && (
                    <div className="project-entry">
                      <div className="gameboy-screen">
                        <img src={nextDisplayed[idx].image || "/placeholder.svg"} alt={nextDisplayed[idx].title} />
                      </div>
                      <h3>{nextDisplayed[idx].title}</h3>
                      <p className="project-description">{nextDisplayed[idx].description}</p>

                      <div className="gameboy-controls">
                        <div className="tech-stack">
                          {nextDisplayed[idx].technologies &&
                            parseTechnologies(nextDisplayed[idx].technologies).map((tech, index) => (
                              <button key={index} className="tech-badge" title={tech}>
                                {tech}
                              </button>
                            ))}
